...
empty :: C.AnnI a (Tree k v) m
empty p = p Leaf

singleton :: Monad m => k -> v -> C.AnnI a (Tree k v) m
singleton k v p =
  do l0 <- p Leaf
     l1 <- p Leaf
     p (Branch k v l0 l1)

triplet :: (Applicative m, Monad m) => k -> v -> k -> v -> k -> v -> C.AnnI a (Tree k v) m
triplet a0 b0 a1 b1 a2 b2 p =
  (| p (| (Branch a1 b1)
  (| p (| (Branch a0 b0) (p Leaf) (p Leaf) |) @ |)
  (| p (| (Branch a2 b2) (p Leaf) (p Leaf) |) @ |) |) @ |)

lookup :: (Monad m, Applicative m, Ord k) => k -> C.AnnO a (Tree k v) m (Maybe v)
...
size :: (Num c, Applicative m) => C.AnnO a (Tree k b) m c
size _ Leaf =    (| 0 |)
size q t    = (| (| 1 |) + (| q (leftT t) + q (rightT t) |) |)

depth :: (Ord c, Num c, Applicative m, Monad m) => C.AnnO a (Tree k v) m c
depth _ Leaf =    (| 0 |)
depth q t    = (| (| 1 |) + (| max (q (leftT t)) (q (rightT t)) |) |)

alter :: (Monad m, Ord k) => (Maybe v -> Maybe v) -> k -> C.AnnIO a (Tree k v) m
